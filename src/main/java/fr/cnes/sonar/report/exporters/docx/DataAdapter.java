...
public final class DataAdapter {
    ...
    */
    ...
    /**
     * Build rule list to display the pourcentage of issue by tools.
     * @param report Report from which to extract resources
     * @param language CodingLanguage language defintion
     * @return list of lists of strings : Line to display
     */
    public static List<List<String>> getTransgressionSynthesis(Report report, CodingLanguage language) {

        List<List<String>> results =  new ArrayList<List<String>>() ;
        Map<String,Integer> ruleByTypeNumber = new HashMap<String,Integer>();
        
        int totalRuleNumber = 0;
        int nb = 0;
        int index = 0;
        boolean found;
        String rule;
        ...
        for(Issue issue : report.getIssues()) 
    	{
        	//check if it is generated by required tools
        	for(CodingLanguageTool tool : language.getTools()) {
        		// check if it a rule type associated to tools
        		found = false ;
            	for(String ruleType : tool.getRuleTypes())
            	{
            		rule = issue.getRule();
            		index = rule.indexOf(ruleType);
            		if(index != -1 ) 
            		{
            			found  = true;
            	...
            	// if found count the number
            	if ( found ) {
	            	// count the found rules
	    			totalRuleNumber++;
	    			// if not already rule found for this type 
	    			if ( ruleByTypeNumber.containsKey(tool.getName()) == false) {
	    				ruleByTypeNumber.put(tool.getName(), new Integer(1));
	    			}
	    			else {
	    				//increment
	    				nb = ruleByTypeNumber.get(tool.getName()).intValue() + 1;
	    				ruleByTypeNumber.put(tool.getName(), new Integer(nb));
	    			}
            	}
        	}
        }
        ...
        for(CodingLanguageTool tool : language.getTools()) { 
         	...
         	// Check if the tool is recognized
         	if ( ruleByTypeNumber.containsKey(tool.getName()) == true) {
         		line = new ArrayList<>();
         		line.add(tool.getLabel()); 
         		nb = ruleByTypeNumber.get(tool.getName()).intValue();
         		total = ((double)nb/totalRuleNumber);
         		line.add(String.valueOf(df1.format(nb)));
            	line.add(String.valueOf(df.format(total)));
    ...
    /**
     * Build rule list to display depending of the required ruleType
     * @param report report from which to extract resources
     * @param tool CodingLanguageTool Tool to which we want the trangressed rules
     * @return list of lists of strings : Line to display
     */
    public static List<List<String>> getRulesTransgression(Report report, CodingLanguageTool tool) {

    	// For sorting issue type by number of occurence, use treemap.
    	// orderedIssueMap<Number of occurence , rule>
    	Map<Integer, List<String>> orderedIssueMap = new TreeMap<Integer, List<String>>(Collections.reverseOrder());
    	// foundRules<rule, line to display
        Map<String,List<String>> foundRules =  new HashMap<String,List<String>>() ;
        List<List<String>> result = new ArrayList<List<String>>();
        
        String ruleTemp = null;
        List<String> ruleData;
        List<String> ruleList;
        
        String countString = null;
        int count;	
        //position of issue number in list
        ...
        for(Issue issue : report.getIssues())
        {
        	// If the issue have the required rule type
        	ruleTemp = issue.getRule();
        	for (String ruleType : tool.getRuleTypes()) {
		    	index = ruleTemp.indexOf(ruleType);
		    	if(index != -1 )
		    	{
		    		// If it is the first time the rule  is found
		    		if ( foundRules.containsKey(ruleTemp) == false) {
		    			ruleData = new ArrayList<String>();
		    			// Build information line
						ruleData.add(ruleTemp.replace(ruleType, "")+ " - "+issue.getMessage());
						ruleData.add(issue.getType());
						ruleData.add(issue.getSeverity());
						ruleData.add("1");
						//Save number of occurance
						if ( orderedIssueMap.containsKey(new Integer(1)) == false) {
							ruleList = new ArrayList <String>();
							ruleList.add(ruleTemp);
							orderedIssueMap.put(new Integer(1), ruleList);
						}
						else {
							orderedIssueMap.get(new Integer(1)).add(ruleTemp);
						}
						//Save data
						foundRules.put(ruleTemp, ruleData);
		    		}
		    		else {
		    			//Increment the number of time the rule is recognized
		    			ruleData = foundRules.get(ruleTemp);
		    			countString = ruleData.get(numberPosition);
		    			count = Integer.parseInt(countString);
		    			//delete old reference
		    			orderedIssueMap.get(new Integer(count)).remove(ruleTemp);
		    			//increment
		    			count++;
		    			//save
		    			if ( orderedIssueMap.containsKey(new Integer(count)) == false) {
							ruleList = new ArrayList <String>();
							ruleList.add(ruleTemp);
							orderedIssueMap.put(new Integer(count), ruleList);
						}
						else {
							orderedIssueMap.get(new Integer(count)).add(ruleTemp);
						}
		    			ruleData.set(numberPosition, String.valueOf(count));
		    		}
		    	}
        	}
        }

        // Return all line found in ordered sort
        Set<Integer> keys = orderedIssueMap.keySet();
        Integer key;
        
        for (Iterator<Integer> iterator = keys.iterator(); iterator.hasNext();) {
          key = (Integer) iterator.next();
          ruleList = orderedIssueMap.get(key);
          
          for (String rule : ruleList) {
        	  result.add(foundRules.get(rule));
          }
        }
    ...
    /**
     * Build table withe language transgression
     * @param report report from which to extract resources
     * @return list of lists of strings : Line to display
     */
    public static List<List<String>> getLangTransgression(Report report) {
        
        Map<String,List<String>> foundLanguages =  new HashMap<String,List<String>>() ;
        
        String languageTemp = null;
        ...
        {
        	// get the language
        	languageTemp = issue.getLanguage();
        	//System.out.println( "^^DataAdapter (" + languageTemp+") ("+issue.getMessage()+")("+issue.getLine()+")("+issue.getKey()+")");
			// If it is the first time the language  is found
    		if ( foundLanguages.containsKey(languageTemp) == false) {
    			languageData = new ArrayList<String>();
    			// Build information line
				languageData.add(languageTemp);
    			languageData.add("1");
				//Save data
				foundLanguages.put(languageTemp, languageData);
    		}
    		else {
    			//Increment the number of time the rule is recognized
    			languageData = foundLanguages.get(languageTemp);
    			countString = languageData.get(1);
    			count = Integer.parseInt(countString) + 1;        			
    			languageData.set(1, String.valueOf(count));
    		}
    	}

        // Return all line found
        return new ArrayList<List<String>>(foundLanguages.values());
    }
